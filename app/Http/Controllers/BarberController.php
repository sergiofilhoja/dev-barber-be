...
class BarberController extends Controller
{
    ...
    */
    ...
    private function searchGeo($address) {
        ...
        $url = 'http://maps.googleapis.com/maps/api/geocoding/json?address='.$address.'&key='.$key;
        $ch = curl_init();
    ...
    public function one($id) {
        ...
        if ($barber) {
            ...
            // Juntando e formando lista de disponibilidade real
            for ($q=0;$q<20;$q++) {
    ...
    public function setAppointment($id, Request $request) {
        ...
        // 1. Verifica se o servico do barbeiro existe
        ...
        if ($barberService) {
            // 2. Verifica se a data é real
            $apDate = $year.'-'.$month.'-'.$day.' '.$hour.':00:00';
            
            if (strtotime($apDate) > 0) {
                // 3. Verifica se o barbeiro já existe agendamento no dia/hora
                ...
                if ($apps === 0) {
                    // 4. Verifica se o barbeiro atende na data/hora desejada
